...
- **transactions** — движение реальных денег: `kind` ('income'|'expense'|'salary'|'refund'), `amount`, `date`, `accountId`, `categoryId`, `clientId?`, `payerName?`, `teacherId?`, `comment`, `sourceId?` (след миграции)
...
- **Баланс компании** = сумма по кассам: доходы − расходы − ЗП − возвраты.
